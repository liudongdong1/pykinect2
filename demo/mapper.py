...
def color_2_depth_space(kinect, color_space_point, depth_frame_data, show=False, return_aligned_image=False):
    ...
    if show:
        ...
        cv2.imshow('testimg', cv2.flip(color_img[colorYs, colorXs, :], 1))
        cv2.waitKey(3000)
    if return_aligned_image:
...
if __name__ == '__main__':
    ...
    while True:
        if kinect.has_new_depth_frame():
            ...
            img = depth_2_color_space(kinect, _DepthSpacePoint, kinect._depth_frame_data, show=True, return_aligned_image=True)
            ...
            if idx > 0:
                cv2.imwrite('./result/rgb'+'{:06d}'.format(idx)+'.jpg', colorImage)
                cv2.imwrite('./result/depth'+'{:06d}'.format(idx)+'.jpg', img)
            idx += 1
        # Quit using q
